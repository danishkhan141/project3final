				...
				<table class="table table-sm table-hover" border="4"
					...
					%><tbody>
						<tr style="background-color: white">
							...
							|| roleDTO.getId() == RoleDTO.STUDENT || roleDTO.getId() == RoleDTO.GUEST) ? "disabled"
									: ""%>></td>
							<td><%=index++%></td>
							<td><%=roleDTO.getRoleName()%></td>
							<td><%=roleDTO.getDescription()%></td>
							<%
								if ((roleDTO.getId() == RoleDTO.ADMIN || roleDTO.getId() == RoleDTO.STAFF
												|| roleDTO.getId() == RoleDTO.STUDENT || roleDTO.getId() == RoleDTO.GUEST)) {
							%>
							<td>-----</td>
							<%
								} else {
